 ...
 *
 */
class ensaf_Tab_Builder extends Widget_Base {

	public function get_name() {
		return 'ensaftabbuilder';
	}
	public function get_title() {
		return __( 'Tab Builder', 'ensaf' );
	}
	...
	protected function register_controls() {
		...
		ensaf_media_fields($this, 'shape', 'Choose Subtitle Shape', ['2'] );
		ensaf_general_fields($this, 'subtitle', 'Subtitle', 'TEXTAREA2', 'Explore Recent Projects', ['2'] );
		ensaf_general_fields($this, 'title', 'Title', 'TEXTAREA', 'We have done the latest projects', ['2'] );

		$repeater = new Repeater();

		ensaf_general_fields( $repeater, 'title', 'Tab Builder Title', 'TEXTAREA2', 'Tab 1' );

		$repeater->add_control( 
			'ensaf_tab_builder_option',
			[
				'label'     => __( 'Tab Name', 'ensaf' ),
				'type'      => Controls_Manager::SELECT,
				'options'   => $this->ensaf_tab_builder_choose_option(),
				'default'	=> ''
			]
		);
		...
		ensaf_common_style_fields( $this, '01', 'Subtitle', '{{WRAPPER}} .sub-title', ['2'],'--theme-color' );
		ensaf_common_style_fields( $this, '02', 'Title', '{{WRAPPER}} .sec-title', ['2'] );

    }

	public function ensaf_tab_builder_choose_option(){

		$ensaf_post_query = new WP_Query( array(
			'post_type'				=> 'ensaf_tab_builder',
			'posts_per_page'	    => -1,
		) );

		$ensaf_tab_builder_title = array();
		$ensaf_tab_builder_title[''] = __( 'Select a Tab','Foodelio');

		while( $ensaf_post_query->have_posts() ) {
			$ensaf_post_query->the_post();
			$ensaf_tab_builder_title[ get_the_ID() ] =  get_the_title();
		}
		wp_reset_postdata();

		return $ensaf_tab_builder_title;
			...
			echo '<div class="tab-content">';
				...
				foreach( $settings['tab_builder_repeater'] as $data ){
					...
					echo '<div class="tab-pane fade '.esc_attr($active).'" id="nav-'.esc_attr($x).'" role="tabpanel" aria-labelledby="nav-'.esc_attr($x).'-tab">';
						...
						if( ! empty( $data['ensaf_tab_builder_option'] ) ){
							echo $elementor->frontend->get_builder_content_for_display( $data['ensaf_tab_builder_option'] );
						}
					echo '</div>';
				}
			echo '</div>';
